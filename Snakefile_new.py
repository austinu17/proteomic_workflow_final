...
MZMLFILES = glob.glob("{0}/*".format(config["raw"]["location"]))
#print(MZMLFILES)
allowed_exts = [".raw",".RAW"]
...
#        expand("csv/ffm_{dbsearch}_proteinIntensities.csv", dbsearch=config["search_engines"]),
         ...
         expand("mztab/{dbsearch}/{sample}/ffc_idm_results_{sample}.mzTab",dbsearch=config["search_engines"],sample=SAMPLES),
         expand("mztab/{dbsearch}/{sample}/ffc_ff_idm_results_{sample}.mzTab",dbsearch=config["search_engines"],sample=SAMPLES),
         expand("work/{dbsearch}/{sample}/ffc_ff_filt_idmap_{sample}.featureXML",dbsearch=config["search_engines"],sample=SAMPLES),
         expand("work/{dbsearch}/{sample}/ffc_export_{sample}.csv",dbsearch=config["search_engines"],sample=SAMPLES),
         expand("work/{dbsearch}/{sample}/ffc_ff_export_{sample}.csv",dbsearch=config["search_engines"],sample=SAMPLES),
         expand("work/{dbsearch}/{sample}/ffc_{sample}.featureXML",dbsearch=config["search_engines"],sample=SAMPLES),
         expand("work/{dbsearch}/{sample}/ffc_ff_{sample}.featureXML",dbsearch=config["search_engines"],sample=SAMPLES),
#expand("csv/ffidm_{dbsearch}_proteinIntensities.csv", dbsearch=config["search_engines"]),
         #expand("mztab/{dbsearch}/fido_fdr_filt_prot_only.mzTab", dbsearch=config["search_engines"]),
          #expand("mztab/{dbsearch}/fp_proteins.mzTab", dbsearch=config["search_engines"]),
        #expand("work/{dbsearch}/{sample}/fdr_mapalign_merge_{sample}.idXML",dbsearch=config["search_engines"],sample=SAMPLES),
...
# Remove Compression
include: "rules/file_conversion_thermo.py"
...
# Perfrom Post Processing of Database Searches
include: "rules/search_post_processing_perc.py"
...
# Perfrom Feature Finding
include: "rules/feature_finder_identification_merged.py"

# Perfrom Feature Finding
#include: "rules/feature_finder_multiplex.py"

# Perfrom Feature Finding Centroid
include: "rules/feature_finder_centroid.py"

# Quantify Centroid Features 
include: "rules/feature_finder_centroid_quantify.py"
